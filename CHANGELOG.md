...
### Added
- ✅ **Terminated Sync Gap Reconciliation (Idempotent)**
  - Introduced deterministic terminated gap model with stable gap keys.
  - Added single-flight terminated sync execution to avoid concurrent reprocessing.
  - Added session-level and persisted-data duplicate guards for restarted sync flows.

- ✅ **Production Example Flow**
  - Example app now demonstrates explicit production startup sequence:
    - `initializeLogging(...)`
    - `start(...)`
    - `startLogging(...)`
  - Includes low-end-friendly isolate usage pattern and runtime-state visibility.

### Changed
- 📘 **README Production Scope Clarified**
  - Android scope corrected to API 24+.
  - Production readiness guarantees documented (dedupe + ANR safeguards).
  - Quick-start now shows explicit production startup path.

### Tested
- 🧪 Added unit coverage for terminated gap keying/splitting semantics.
- 🧪 Full package test suite passes, including example widget tests.
